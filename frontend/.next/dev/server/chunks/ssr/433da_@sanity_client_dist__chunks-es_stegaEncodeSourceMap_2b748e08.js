...
`,
    ...
    const filterDefault = ({ sourcePath, resultPath, value }) => {
        ...
        return !(
          (sourcePath.at(-2) === "slug" && endPath === "current") ||
          (typeof endPath == "string" && (endPath.startsWith("_") || endPath.endsWith("Id"))) ||
          sourcePath.some((path) => path === "meta" || path === "metadata" || path === "openGraph" || path === "seo") ||
          hasTypeLike(sourcePath) ||
          hasTypeLike(resultPath) ||
    ...
    const ZERO_WIDTHS = [
      ...
      ZERO_WIDTHS_CHAR_CODES = ZERO_WIDTHS.map((x) => String.fromCharCode(x)),
      LEGACY_WIDTHS = [
    ...
    const PREFIX = String.fromCodePoint(ZERO_WIDTHS[0]).repeat(4),
      ALL_WIDTHS = [...ZERO_WIDTHS, ...LEGACY_WIDTHS];
    ...
    function stegaCombine(visible, metadata, skip = "auto") {
      return skip === !0 || (skip === "auto" && !isDateLike(visible) && !isUrlLike(visible))
        ? `${visible}${stegaEncode(metadata)}`
        : visible;
    }
...
//# sourceMappingURL=433da_%40sanity_client_dist__chunks-es_stegaEncodeSourceMap_2b748e08.js.map
