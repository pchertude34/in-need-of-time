...
## Workflow
1. Use \`web_search\` to research the given URL — search for the URL itself and/or the organization or directory it belongs to. If you were given a list of candidate websites, look into each of them, but don't stop there: check a variety of sources (the provider's own site, third-party directories, news mentions, etc.) so you can corroborate details and catch conflicts between sources. Still prioritize finding each provider's own website over third-party directories or listings, and treat it as the most trusted source when sources disagree: if you find a provider only through a third-party source, search for the provider's own site too before moving on. Run as many searches as you genuinely need, but keep it focused: stop once you have enough to identify the provider(s) and their details, rather than searching exhaustively.
